...
const DashUtils = {
  generate_dash_file_from_formats(VideoFormats: any, VideoLength: any) {
    const generatedJSON: any = this.generate_xmljs_json_from_data(
      VideoFormats,
      VideoLength
    );
    return json2xml(generatedJSON);
  },
  generate_xmljs_json_from_data(VideoFormatArray: any, VideoLength: any) {
    const convertJSON = {
      declaration: {
        attributes: {
          version: "1.0",
  ...
  generate_adaptation_set(VideoFormatArray: any) {
    const adaptationSets = [];
    const mimeTypes: any = [];
    const mimeObjects = [[]];
    // sort the formats by mime types
    VideoFormatArray?.forEach((videoFormat: any) => {
      // the dual formats should not be used
      if (
        videoFormat.mimeType?.indexOf("video") != -1 &&
  ...
  generate_representation_audio(Format: any) {
  ...
  generate_representation_video(Format: any) {
    const representation = {
      type: "element",
      name: "Representation",
      attributes: {
